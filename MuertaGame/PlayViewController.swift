...
class PlayViewController: UIViewController{
    ...
    private let images: [String] = ["11","12","13","14","15","16"]
    ...
    private func setupTableImageView() {
        ...
        panelImageView.image = UIImage(named: "tableImage")
        
        [panelImageView.topAnchor.constraint(equalTo: view.topAnchor, constant: 120),
    ...
    private func setupStackView(){
        ...
        let stackViewSwitch = [stackOneRow, stackViewTwoRow,stackViewThreeRow]
        ...
        let oneGroup = [oneButton,twoButton,threeButton,fourButton]
        for i in oneGroup {
            stackOneRow.addArrangedSubview(i)
            i.addTarget(self, action: #selector(onTap), for: .touchUpInside)
        }
        
        let twoGroup = [fiveButton,sixButton,sevenButton,eightButton]
        for i in twoGroup {
            stackViewTwoRow.addArrangedSubview(i)
            i.addTarget(self, action: #selector(onTap), for: .touchUpInside)
        }
        
        let threeGroup = [nineButton,tanButton,elevenButton,twelveButton]
        for i in threeGroup {
        ...
        let group = [oneButton, twoButton, threeButton,fourButton, fiveButton, sixButton, sevenButton, eightButton, nineButton, tanButton, elevenButton, twelveButton]
    ...
    @objc private func onTap(sender: UIButton) {
        ...
        if let firstSelectedImageString {
            if firstSelectedImageString == selectedImage {
                ...
                let group = [self.oneButton, self.twoButton, self.threeButton,self.fourButton, self.fiveButton, self.sixButton, self.sevenButton, self.eightButton, self.nineButton, self.tanButton, self.elevenButton, self.twelveButton]
